...
class Auth extends MY_Controller {
	...
	function login()
	{
        ...
        if ($this->form_validation->run() == true) {
			...
			//$passkey = md5($this->input->post('username').date("Y-m-d H:i:s").$this->input->post('password'));
			//die($passkey);
	...
	function logout()
	{
		...
		//clear user last page
		$this->db->query("UPDATE users SET last_page=NULL WHERE id='". $this->session->userdata('user_id') ."'");
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template('auth/deactivate_user', $data);
		}
		else
		{
	...
	function create_user()
	{
		...
		else
		{
			...
			$this->template('auth/create_user', $data);
		}
	}
	...
	function create_group()
	{
		...
		else
		{
			...
			$this->template('auth/create_group', $data);
		}
	}
